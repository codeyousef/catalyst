...
/// use std::fs::File;
/// use std::io::Write;
/// use catalyst_proxy::plugin::wasi::load_volt;
...
pub fn start_volt(
    ...
    linker.func_wrap("lapce", "host_handle_stderr", move || {
        if let Ok(msg) = wasi_read_string(&stderr) {
            tracing_log::log::log!(target: &format!("catalyst_proxy::plugin::wasi::{}::{}", plugin_meta.author, plugin_meta.name), tracing_log::log::Level::Debug, "{msg}");
        }
    })?;
