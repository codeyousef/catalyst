...
use lsp_types::{
    ...
    CodeActionProviderCapability, DidChangeTextDocumentParams, DidSaveTextDocumentParams,
    DocumentSelector, FoldingRangeProviderCapability, HoverProviderCapability,
    ImplementationProviderCapability, InitializeResult, LogMessageParams, MessageType, OneOf,
    ProgressParams, PublishDiagnosticsParams, Range, Registration, RegistrationParams,
...
impl PluginHostHandler {
    ...
    pub fn handle_notification(
        ...
        match method.as_str() {
            ...
            StartLspServer::METHOD => {
                self.core_rpc.log(
                    catalyst_rpc::core::LogLevel::Warn,
                    format!(
                        "[{}] Usage of startLspServer as a notification is deprecated.",
                        self.volt_display_name
                    ),
                    Some(format!("catalyst_proxy::plugin::psp::{}::{}::StartLspServer", self.volt_id.author, self.volt_id.name)),
                );
            ...
            LogMessage::METHOD => {
                ...
                self.catalog_rpc.core_rpc.log_message(
                    message,
                    format!(
                        "catalyst_proxy::plugin::psp::{}::{}::LogMessage",
            ...
            _ => {
                self.core_rpc.log(
                    ...
                    Some(format!(
                        "catalyst_proxy::plugin::psp::{}::{}::{method}",
                        self.volt_id.author, self.volt_id.name
                    )),
                );
            }
