...
use catalyst_rpc::plugin::VoltMetadata;
...
#[test]
fn test_load_volt() {
    let catalyst_proxy_dir = std::env::current_dir()
        .expect("Can't get \"catalyst-proxy\" directory")
        .join("src")
        .join("plugin")
        .join("wasi")
        .join("plugins");
    ...
    let path = catalyst_proxy_dir.join("some-path");
    ...
    if let Ok(volt_metadata) = load_volt(&catalyst_proxy_dir) {
        panic!(
            "Unexpected result from `catalyst_proxy::plugin::wasi::load_volt` function: {volt_metadata:?}"
        );
    }

    // Invalid file (not readable into a string)
    // Making sure the file exists
    let path = catalyst_proxy_dir.join("smiley.png");
    ...
    let parent_path = catalyst_proxy_dir.join("some_author.test-plugin-one");
    ...
    let parent_path = catalyst_proxy_dir.join("some_author.test-plugin-two");
    ...
    let parent_path = catalyst_proxy_dir.join("some_author.test-plugin-three");
