...
fn get_github_api(url: &str) -> Result<String> {
    let user_agent = format!("Lapce/{}", catalyst_core::meta::VERSION);
    let resp = catalyst_proxy::get_url(url, Some(user_agent.as_str()))?;
...
pub fn find_grammar_release() -> Result<ReleaseInfo> {
    ...
    use catalyst_core::meta::{RELEASE, ReleaseType, VERSION};
...
fn download_release(
    ...
    for asset in &release.assets {
        if asset.name.starts_with(file_name) {
            let mut resp =
                catalyst_proxy::get_url(&asset.browser_download_url, None)?;
            if !resp.status().is_success() {
