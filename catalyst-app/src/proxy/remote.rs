...
fn download_remote(
    ...
    let script_install = match platform {
        HostPlatform::Windows => {
            ...
            let remote_proxy_script = "${env:TEMP}\\catalyst-proxy.ps1";
    ...
    if !script_install.success() {
        ...
        if !cmd.success() {
            let proxy_filename = format!("catalyst-proxy-{platform}-{architecture}");
            ...
            let mut resp =
                catalyst_proxy::get_url(url, None).expect("request failed");
