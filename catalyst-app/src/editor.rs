...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let catalyst_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_move_command(
        &self,
        movement: &catalyst_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
            ...
            FocusCommand::JumpToNextSnippetPlaceholder => {
                self.snippet.update(|snippet| {
                    if let Some(snippet_mut) = snippet.as_mut() {
                        ...
                        {
                            let mut selection =
                                catalyst_core::selection::Selection::new();
                            let region = catalyst_core::selection::SelRegion::new(
                                *start, *end, None,
                            );
                        ...
                        if current > 0 {
                                ...
                                snippet_mut.get(current - 1)
                            {
                                let mut selection =
                                    catalyst_core::selection::Selection::new();
                                let region =
                                    catalyst_core::selection::SelRegion::new(
                                        *start, *end, None,
                                    );
                                selection.add_region(region);
                                self.cursor().update(|cursor| {
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &catalyst_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &catalyst_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn apply_completion_item(&self, item: &CompletionItem) -> anyhow::Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .flatten()
            .map(|edit| {
                let selection = catalyst_core::selection::Selection::region(
                    buffer.offset_of_position(&edit.range.start),
                    buffer.offset_of_position(&edit.range.end),
                );
                (selection, edit.new_text.as_str())
            })
            .collect::<Vec<(catalyst_core::selection::Selection, &str)>>();
    ...
    pub fn completion_apply_snippet(
        ...
        let mut selection = catalyst_core::selection::Selection::new();
        let (_tab, (start, end)) = &snippet_tabs[0];
        let region = catalyst_core::selection::SelRegion::new(*start, *end, None);
        selection.add_region(region);
        cursor.set_insert(selection);
    ...
    fn search_whole_word_forward(&self, mods: Modifiers) {
        ...
        if let Some((start, _end)) = next {
            self.run_move_command(
                &catalyst_core::movement::Movement::Offset(start),
                None,
                mods,
            );
        }
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            catalyst_core::cursor::CursorMode::Normal(offset) => {
                catalyst_core::selection::SelRegion::caret(*offset)
            }
            catalyst_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            } => catalyst_core::selection::SelRegion::new(
            ...
            catalyst_core::cursor::CursorMode::Insert(selection) => {
