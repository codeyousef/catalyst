...
use anyhow::Result;
use catalyst_core::{command::EditCommand, directory::Directory, mode::Mode};
use catalyst_proxy::plugin::{download_volt, volt_icon, wasi::find_all_volts};
use catalyst_rpc::{
...
impl PluginData {
    ...
    pub fn volt_installed(&self, volt: &VoltMetadata, icon: &Option<Vec<u8>>) {
        ...
        if !is_latest {
            ...
            std::thread::spawn(move || {
                let info: Option<VoltInfo> = catalyst_proxy::get_url(url, None)
                    .ok()
    ...
    fn load_icon(volt: &VoltInfo) -> Result<VoltIcon> {
        ...
        let content = match cache_content {
            ...
            None => {
                let resp = catalyst_proxy::get_url(&url, None)?;
    ...
    fn query_volts(query: &str, offset: usize) -> Result<VoltsInfo> {
        ...
        let plugins: VoltsInfo = catalyst_proxy::get_url(url, None)?.json()?;
        Ok(plugins)
    }
