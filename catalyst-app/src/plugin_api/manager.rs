...
use crate::plugin_api::{
    AiAssistantPlugin, SidebarPanelRegistry, McpServerRegistry,
    AiPluginInfo, SidebarPanelInfo, McpServerInfo
};
...
impl PluginManager {
    ...
    pub fn load_plugins_from_directory(&mut self, directory: &str) -> Result<()> {
        ...
        tracing::info!("Plugin loading from directory '{}' is not yet implemented", directory);
        
        Ok(())
    }
    
    /// Register an AI assistant plugin
    pub fn register_ai_assistant(&mut self, id: String, plugin: Arc<dyn AiAssistantPlugin>) -> Result<()> {
        if self.ai_assistants.contains_key(&id) {
            return Err(anyhow::anyhow!("AI assistant with id '{}' is already registered", id));
        }
