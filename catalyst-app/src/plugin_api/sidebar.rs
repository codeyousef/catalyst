...
pub trait SidebarPanelPlugin: Send + Sync {
...
impl SidebarPanelRegistry {
    ...
    pub fn register_panel(&mut self, id: String, panel: Box<dyn SidebarPanelPlugin>) -> Result<()> {
        if self.panels.contains_key(&id) {
            return Err(anyhow::anyhow!("Panel with id '{}' is already registered", id));
        }
    ...
    pub fn get_panel_mut(&mut self, id: &str) -> Option<&mut dyn SidebarPanelPlugin> {
