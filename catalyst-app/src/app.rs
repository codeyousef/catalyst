...
struct Cli {
    ...
    #[clap(value_parser = catalyst_proxy::cli::parse_file_line_column)]
...
pub fn launch() {
    ...
    if let Err(err) = catalyst_proxy::register_lapce_path() {
        tracing::error!("{:?}", err);
    }
...
pub fn try_open_in_existing_process(
    ...
    catalyst_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    catalyst_rpc::stdio::read_msg(&mut reader)?;
